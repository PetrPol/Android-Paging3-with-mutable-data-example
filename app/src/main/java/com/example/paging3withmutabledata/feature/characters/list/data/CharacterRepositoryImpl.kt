...
import com.example.paging3withmutabledata.feature.characters.list.data.cache.CachedCharacterSearchPager
...
class CharacterRepositoryImpl(
    ...
    override fun getCharacterSearchStream(coroutineScope: CoroutineScope): SearchPagingHandle<Character> {
        return CachedCharacterSearchPager(characterCache, coroutineScope) { query ->
            DefaultPagingSource { pagingRequest ->
                remoteDataSource.searchForCharacters(pagingRequest, query)
            }
        }.createPagingHandle()
    }
